...
def migrate():
    ...
    ops = []
    phone_map = {}   # phone_10 -> account doc
    ...
    for m in merchants_list:
        ...
        if phone_10 in phone_map:
            ...
            # Prefer merchant token if user token is empty
            if not phone_map[phone_10].get("token"):
                phone_map[phone_10]["token"] = m.get("token", "")
        else:
            # Merchant only — no user record
            doc = {
                "phone":         phone_10,
                "phone_variants": phone_variants(phone_10),
                "name":          m.get("name", ""),
    ...
    # ── Upsert into accounts collection ─────────────────────────────────────
    ...
    print("\nCreating indexes on accounts collection...")
    db.accounts.create_index("phone",       unique=True, background=True)
    ...
    print("\n✅ Migration complete!")
    print("   Original 'users' and 'merchants' collections are UNTOUCHED.")
    print("   Verify accounts collection, then update backend to use db.accounts.")
